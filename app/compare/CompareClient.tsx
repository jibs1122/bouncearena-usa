...
function Tip({ text }: { text: string }) {
  ...
  return (
    <span className="relative inline-block ml-0.5 align-middle"
      ...
      {show && (
        <span className="absolute bottom-full left-1/2 -translate-x-1/2 mb-1.5 w-48 px-2 py-1.5 bg-black/80 text-white text-[10px] leading-snug rounded-lg pointer-events-none z-50 whitespace-normal">
          {text}
        </span>
      )}
    </span>
                  ...
                  <Fragment key={key}>
                    <tr
                      ...
                      <td className="px-3 py-3 align-top whitespace-nowrap">
                        {row.minPrice !== null ? (
                          <>
                            <span className="font-semibold text-black">{fmtPrice(row.minPrice)}</span>
                            {row.maxPrice !== null && row.maxPrice !== row.minPrice && (
                              <span className="text-black/40 text-sm"> to {fmtPrice(row.maxPrice)}</span>
                            )}
                            {row.shopUrl && (
